...
selection=$(echo -ne "$final_rofi" | rofi -markup-rows -theme ~/.config/rofi/themes/nord.rasi -dmenu -eh 3 -a "$active_list" -u "$urgent_list" -sep '\x0f' -p "Notification Center" -no-fixed-num-lines -lines 8 -i -no-config)
